angular.module('app').service('ServiceService', function ($http, $q, $state, FeedbackService, ErrorsFactory) {
  ...
  this.getProcessDefinitions = function (oServiceData, latest) {
    ...
    if(oServiceData && oServiceData.oData && oServiceData.oData.processDefinitionId){
      data['key'] = oServiceData.oData.processDefinitionId.split(':')[0];
    }
  ...
  this.getMinJustServiceUrl = function (oNAIS) {
    return $http.get('./api/process-form/get-nais-service', {
      params : {
        sID_NAIS_Service_code: oNAIS.sServiceCode,
        sID_NAIS_Application_id: oNAIS.sApplicationId
      }
    })
  }
