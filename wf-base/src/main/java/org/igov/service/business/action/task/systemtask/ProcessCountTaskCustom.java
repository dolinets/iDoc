...
//		String snCount = getActionProcessCount(execution.getProcessDefinitionId(), null);
		...
		if (snCount != null) {
                    ...
                    String sVariableName = sReturn.substring(sReturn.indexOf("[") + 1, sReturn.indexOf("]"));
                    
                    sReturn = sReturn.replace("[" + sVariableName + "]", snCount);
                    
                    runtimeService.setVariable(oExecution.getProcessInstanceId(), sVariableName, sReturn);
                    ...
                    for (Task oTask : aTask) {
                        ...
                        for (FormProperty oFormProperty : oTaskFormData.getFormProperties()) {
                            if (oFormProperty.getId().equals(sVariableName)) {
                                LOG.info("Found form property with the id " + sVariableName + ". Setting sReturn={}", sReturn);
                        ...
                        for (FormProperty oFormProperty : oStartFormData.getFormProperties()) {
                            if (oFormProperty.getId().equals(sVariableName)) {
                                LOG.info("Found start form property with the id " + sVariableName + ". Setting sReturn={}", sReturn);
                                if (oFormProperty instanceof FormPropertyImpl) {
                                    ((FormPropertyImpl) oFormProperty).setValue(sReturn);
                                }
                            }
