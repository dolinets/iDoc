...
public class ProcessSubjectDaoImpl extends GenericEntityDao<Long, ProcessSubject> implements ProcessSubjectDao {
    ...
    @Transactional
    @Override
    public ProcessSubject setProcessSubjectStatusAndReport(String snID_Process_Activiti,
            ProcessSubjectStatus processSubjectStatus, String sReport
    ) {
        ProcessSubject processSubject = findByProcessActiviti(snID_Process_Activiti);
        if (processSubject != null) {
            processSubject.setoProcessSubjectStatus(processSubjectStatus);
 ...
 @Override
    public ProcessSubject findByProcessIdLoginAndLoginRole(String snID_Process_Activiti, String sLogin,
            String sLoginRole) {
        ProcessSubject oProcessSubject;
        ...
        criteria.add(Restrictions.eq("sLoginRole", sLoginRole));

        oProcessSubject = (ProcessSubject) criteria.uniqueResult();
        LOG.debug("findByProcessActivitiIdAndLogin: oProcessSubject={} ", oProcessSubject);
        
        return oProcessSubject;
    }
    
    @Override
    public List<ProcessSubject> findByProcessActivitiIdAndLoginRole(String snID_Process_Activiti, String sLoginRole) {
        Criteria criteria = getSession().createCriteria(ProcessSubject.class);

        criteria.add(Restrictions.eq("snID_Process_Activiti", snID_Process_Activiti));
        criteria.add(Restrictions.eq("sLoginRole", sLoginRole));

        List<ProcessSubject> aProcessSubject = criteria.list();
        LOG.debug("findByProcessActivitiIdAndLogin: aProcessSubject={} ", aProcessSubject);

        return aProcessSubject;
    }
    
    @Override
    public ProcessSubject findByProcessAndLogin(String snID_Process_Activiti, String sLogin) {
        Criteria criteria = getSession().createCriteria(ProcessSubject.class);

        criteria.add(Restrictions.eq("snID_Process_Activiti", snID_Process_Activiti));
        criteria.add(Restrictions.eq("sLogin", sLogin));

        return (ProcessSubject) criteria.uniqueResult();
    }
}
