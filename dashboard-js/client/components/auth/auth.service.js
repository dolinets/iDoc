  ...
  .factory('Auth', function Auth($location, $rootScope, $http, Base64, $cookieStore, $q) {
    ...
    return {
      ...
      getCurrentUser: function () {
        var tmpUser = $cookieStore.get('user');
        if(!currentUser.roles){
          currentUser.roles = angular.fromJson(window.localStorage.getItem('iGovUserRoles'));
        }
        return currentUser !== tmpUser ? tmpUser : currentUser;
      },
      ...
      isSuperAdmin: function () {
        return this.hasOneOfRoles('superadmin');
      },
      ...
      hasOneOfRoles: function () {
        ...
        if (currentUser && currentUser.id && arguments && arguments.length > 0) {
