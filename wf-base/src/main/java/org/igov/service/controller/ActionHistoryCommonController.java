...
public class ActionHistoryCommonController {

    @Autowired
    private ActionEventHistoryService oActionEventHistoryService;

    private static final Logger LOG = LoggerFactory.getLogger(ActionHistoryCommonController.class);
    ...
    HistoryEvent_Service addHistoryEvent(
            ...
            @ApiParam(value = "ИД-номер", required = false) @RequestParam(value = "nID_Subject", required = false) String snID_Subject,
            ...
            @ApiParam(value = "числовой код, который соответсвует типу события", required = false) @RequestParam(value = "nID_HistoryEventType", required = false) String snID_HistoryEventType,
            ...
            @ApiParam(value = "Сумма платежа айпей", required = false) @RequestParam(value = "sIpay_Amount", required = false) String sIpay_Amount,
        ...
        LOG.info("addHistoryEvent was started with params: sID_Order: {} nID_Subject : {} sUserTaskName: {} "
                ...
                sID_Order, snID_Subject, sUserTaskName, nID_Service, nID_ServiceData, nID_ServiceData, nID_Region,
                sID_UA, soData, sToken, sHead, sBody, nID_Proccess_Feedback, nID_Proccess_Escalation, nID_StatusType);
        
        Long nID_Subject = null;
        Long nID_HistoryEventType = null;
        
        if(snID_Subject != null){
            nID_Subject = Long.parseLong(snID_Subject);
        }
        
        if(snID_HistoryEventType != null){
            nID_HistoryEventType = Long.parseLong(snID_HistoryEventType);
        }
