...
exports.authenticate = function (req, res) {
  ...
  async.waterfall([
    ...
    function (jsessionCookie, callback) {
        //config.activiti.session.sessionIdle,
        //    config = require('../../config/environment');
        if(config.bTest){
            callback(null, jsessionCookie);
        }
        //https://gamma.test.region.igov.org.ua/wf/service/subject/getSubjectHuman?sID_Group_Activiti=MJU_Dnipro_Top7_Dep1
        var options = {
          path: 'subject/getSubjectHuman',
          query: {
            sID_Group_Activiti: checkLogin.query.sLogin
          },
          json: true
        };
      activiti.get(options, function (error, statusCode, result) {
        if (error) {
          callback(error, null);
            ...
            if (result && result.oSubjectHuman && result.oSubjectHuman.oServer) {
                var nID = result.oSubjectHuman.oServer.nID;
                var nID_Server = config.activiti.nID_Server;
                if(nID && nID!=nID_Server){
                    var authError = {
                      message: 'Відмовлено у авторізаціі. Ви намагаєтесь увійти у чужий('+nID_Server+') Вашому('+nID+') сервер групи підприємств!',
                      serverMessage: '',
                      code: 0,
                      statusCode: statusCode
                    };
    ...
    if (error) {
      ...
      res.cookie('referent', JSON.stringify({
        email : result.userResult.email,
        firstName : result.userResult.firstName,
        id : result.userResult.id,
        lastName : result.userResult.lastName,
