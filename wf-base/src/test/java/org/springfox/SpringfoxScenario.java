...
public class SpringfoxScenario {
    ...
    public static final String ASCIIDOC_CONTEXT = "/service/api-docs";
    public static final String ASCIIDOC_PATH = "src/main/asciidoc";
	...
	printDocs(ApplicationSwaggerConfig.GROUP_COMMON);
	printDocs(ApplicationSwaggerConfig.GROUP_PROJECT);
    }

    private void printDocs(String group) {
	try {
	    String sPath = (ASCIIDOC_PATH + File.separator + group).toLowerCase().trim();
	    File oFile = new File(sPath);
	    if (!oFile.exists()) {
		oFile.mkdirs();
	    }
	    System.out.println("[Info] createSwaggerDocs: Rebuilbing... sPath = " + sPath);

	    MvcResult result = mockMvc.perform(get(ASCIIDOC_CONTEXT).accept(MediaType.APPLICATION_JSON).param("group", group)
