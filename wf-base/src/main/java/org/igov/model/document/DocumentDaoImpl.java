...
public class DocumentDaoImpl extends GenericEntityDao<Long, Document> implements DocumentDao {
    ...
    public List<Document> getDocuments(Long nID_Subject, List<Long> anID_DocumentType) {
        Criteria oCriteria = createCriteria();
            oCriteria.add(Restrictions.and(
                Restrictions.eq("subject.id", nID_Subject),
                Restrictions.in("documentType.id", anID_DocumentType)
        ));
        return (List<Document>) oCriteria.list();
    }
}
