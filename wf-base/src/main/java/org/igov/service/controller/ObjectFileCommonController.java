...
import org.igov.service.business.util.CharsetDetector;
...
public class ObjectFileCommonController {
    ...
    @Autowired
    CharsetDetector oCharsetDetector;
    ...
    AttachmentEntityI putAttachmentsToExecution(//ResponseEntity
            ...
            @ApiParam(value = "ИД поля формы, к которому загружается файл", required = true) @RequestParam(value = "sID_Field") String sID_Field)
            throws IOException {
    ...
    byte[] getAttachment(
        ...
        httpResponse.setHeader("Content-disposition", "attachment; filename="
                + sFileName);
        httpResponse.setHeader("Content-Type", "application/octet-stream");
        //httpResponse.setCharacterEncoding("windows-1251");
        httpResponse.setContentLength(multipartFile.getBytes().length);
        
        byte[] aMultipartBytes = multipartFile.getBytes();
        
        if(sFileName != null && sFileName.endsWith(".txt")){
            try{
                Charset oCharset = oCharsetDetector.detectCharsets(aMultipartBytes);

                if(oCharset != null){
                    LOG.info("Charset is {}", oCharset.displayName());

                    if(oCharset.displayName().equals("windows-1251")){
                       return (new String(multipartFile.getBytes(), "windows-1251")).getBytes();
                    }

                }else{
                    LOG.info("Charset is null");
                    return (new String(multipartFile.getBytes(), "UTF-8")).getBytes();
                }
            }
            catch(Exception ex){
                LOG.info("Error during charset detecting {}", ex);
            }
        }
        /*httpResponse.setHeader("Content-disposition", "attachment; filename="
                + "test.txt");
