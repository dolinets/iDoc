...
public class ValidationARM {
    ...
    public static boolean isValid(String sValue) {
        return StringUtils.isNotEmpty(sValue) && Objects.nonNull(sValue)
                && !sValue.equals("null");
    }
     ...
     * @return
     */
    public static String getValidSizePrilog(String data) {
        String prilog = "";
        if (isValid(data)) {
            if (data.length() > 200) {
    ...
    public static DboTkModel fillModel(String soData_Value_Result) {
        ...
        dataForTransferToArm.setDataBB(isValid(data.get("DataBB")) ? parseDate(data.get("DataBB"), formats) : null);
        ...
        dataForTransferToArm.setOut_number(isValid(data.get("Out_number")) ? data.get("Out_number") : null);
        if (dataForTransferToArm.getOut_number() == null) {
            throw new IllegalArgumentException("Out_number = " + dataForTransferToArm.getOut_number());
        }
    ...
    public static Date Date_EZ(Date dDateIn, Date dDateEZ) {
        LOG.info("Welcome to create Date_EZ");
        Date dReturn = null;
        ...
        if (dDateEZ == null) {
            ...
            dCalendar.setTime(dDateIn);
            dCalendar.add(Calendar.DATE, 38);

            if (dCalendar.get(Calendar.DAY_OF_WEEK) == Calendar.SATURDAY) {
                dCalendar.add(Calendar.DATE, 2);
            } else if (dCalendar.get(Calendar.DAY_OF_WEEK) == Calendar.SUNDAY) {
                dCalendar.add(Calendar.DATE, 1);
            }
            dReturn = dCalendar.getTime();
        } else {
            dReturn = dDateEZ;
        }
    ...
    public static Date UpdateDate() {
        LOG.info("Welcome to create UpdateDate");
        Date dUpdateData = new Date();
        return dUpdateData;
    }
