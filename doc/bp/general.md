...
<b>Все типы параметров, которые ссылаются на внешний файл, не переименовывать при переименовании услуги, а только создавать новые.<b/> <br>
...
Было видоизменено следующее: по часам и минутам мы не задаем (т.к. шедуллер у девопов), вместо этого - давать возможность вытягивать любое из значений "nElapsedDays", "nDays", "nElapsedHours", "nDueElapsedHours", "nDueElapsedDays", "nCreateElapsedHours", "nCreateElapsedDays" - непосредственно из самого процесса по имени переменной. Для этого его нужно дописать в soData в csv:
```
455;_doc_test_idoc1;"usertask2";"nElapsedDays >= nDaysLimit";"{nElapsedDays:'sID_Order_Relation', nDaysLimit:0, asRecipientMail:['email'], sEscalationType:'submit, sendMail'}";"escalation/escalation_template.html";1
```
Значение nElapsedDays будет тянуться из переменной sID_Order_Relation (задаваться могут только отрицательный и положительные целые числа)
...
### Доработан функционал эскалаций
Теперь возможно вместо имейл задавать поле с таблицей логинов, по которому автоматически вытянутся имейлы. Для этого, помимо sID переменной-таблицы нужно через запятую указать и поле таблицы, где лежат логины:

```javascript
['sTableAgree,sLogin_Approver']
```

Кроме этого, стало возможно формировать заголовок письма. Для этого в темплейте нужно через || указать текст заголовка. Если в него нужно поместить ид ордера или иди бп - добавить [sID_BP] или [nID_Task], они автозаменятся.

Пример вызова эскалации где вместо имейл прописано поле-таблица с имейлами и задана тема письма:
```xml
459;_doc_btsol_tender2;"usertask2_yellow";"nElapsedDays >= nDaysLimit";"{nDaysLimit:0, asRecipientMail:['sTableAgree,sLogin_Approver'], sEscalationType:'sendMail'}";"escalation/escalation_template.html||Тестовый head : sID_BP: [sID_BP] nID_Task: [nID_Task]";1
```

### Добавлен новый тип эскалации - setUrgent
Применяется к документам, для всех процессов стоящих на указанной ют (т.е. степе) - степ опмечается как срочный для всех участников степа  
Пример:
```xml
460;_doc_test_Extren;"usertask3";"nElapsedDays >= nDaysLimit";"{nDaysLimit:0, sEscalationType:'setUrgent'}";"";1
```
...
### Очистить документы по названию бизнес процесса (метод DEL)
```
https://alpha.test.region.igov.org.ua/wf/service/action/task/deleteHistoricProcessInstance?sID_Process_Def=_doc_etg_outgoing&nLimitCountRowDeleted=150
```
удаляет все документы, включая историю, с сервера alpha.test.region.igov.org.ua по названию БП ```_doc_etg_outgoing```
```
https://alpha.test.region.igov.org.ua/wf/service/process/removeAllProcessLinks
```
```
https://alpha.test.region.igov.org.ua/wf/service/action/task/deleteHistoricProcessInstance?nID_Order=40177501
```
где 40177501 - ид процесса

### [Запросы по заявкам](#requests)
### [Запросы по пользователям](#userrequests)
...
### [Ознакомление с iDoc](#overview)
### [Создание БП](#bpcreation)
...
### [Рассылка почты](#mailscript)
### [Callactivity](#callactivity_)  
### [Скрипты, которые используются в СЕДе](#scriptsusedidoc)
### [Степы, клонирование прав](#steps_cloning)
### [Работа с кнопками](#buttons)


###### overview
### Ознакомление iDoc
Видео ознакомление с системой электронного документооборота iDoc.  
[Видео](https://www.youtube.com/watch?v=KNOjaSPix4U)



###### bpcreation
### Создание БП
Видео ознакомление по созданию БП.  
[Видео](https://www.youtube.com/watch?v=_JOprUblW9g)
...
**cloneDocumentStepSubject(snID_Process_Activiti, sKey_GroupPostfix, sKey_GroupPostfix_New, sKey_Step, flag)**
flag = {true, false} - принимает значение false, если клонирование прав проходит только один раз, т.е. на этот шаг возврата нет, нет реклонирования, значение true, если есть возврат и реклонирование. Если параметр не указан, то это равносильно true.  
пример вызова:  
```javascript
var snID_Process_Activiti = execution.getProcessInstanceId()
var sKey_GroupPostfix_New = execution.getVariable('sLogin_AutorResolution')
var oResult = documentStepService.cloneDocumentStepSubject(snID_Process_Activiti, '_default_MJU', sKey_GroupPostfix_New, 'step_1', false)
```
...
**isDocumentStepSubmitedAll(String nID_Process, String sLogin, String sKey_Step)**  

<a href="https://api.tech.igov.org.ua/wf-central/central-igov.html#_проверка_подписано_ли_ЕЦП">Прочитать описание в API</a>
...
```
### Отмена подписи на степе документа (подписант остается)
**cancelDocumentSubmit(String nID_Process, String sKey_Step, String sKey_Group)**  

<a href="https://api.tech.igov.org.ua/wf-central/central-igov.html#_Отмена_сабмиченности_и_подписанности_документа">Прочитать описание в API</a>
...
```
### Удаление подписанта
**removeDocumentStepSubject(String nID_Process, String sKey_Step, String sKeyGroup)**  

<a href="https://api.tech.igov.org.ua/wf-central/central-igov.html#_Удаление_подписанта_документа">Прочитать описание в API</a>
...
**delegateDocumentStepSubject(String snID_Process_Activiti, String sKey_Group, String sKey_Group_Delegate, String sKey_Step) **
<br>
<a href="https://api.tech.igov.org.ua/wf-central/central-igov.html#_Делегировать_подписанта_документа">Прочитать описание в API</a>


### Синхронизация сабмитеров на степе по полю
**syncDocumentSubmitedsByField(String snID_Process_Activiti, String sKey_Group_Default, String sID_Field, String sID_FieldTable, String sKey_Step, boolean bReClone)** ,

<br><a href="https://api.tech.igov.org.ua/wf-central/central-igov.html#_Синхронизация_сабмитеров_на_степе_по_полю">Прочитать описание в API</a>

Пример вызова:
```javascript
var snID_Process_Activiti = execution.getProcessInstanceId()
var oSwitchStep12 = documentStepService.setDocumentStep(snID_Process_Activiti, 'step_3')
var oCloneRightsAgree = documentStepService.syncDocumentSubmitedsByField(snID_Process_Activiti,  '_default_btsol', 'sTable_Approve', 'sLogin_Approver', 'step_3', true)
```
...
**getCatalogSubjectGroupsTree(String sID_Group_Activiti,Long deepLevel, String sFind, Boolean bIncludeRoot, Long deepLevelWidth,String sSubjectType)**
Вызов Human:
```
https://alpha.test.region.igov.org.ua/wf/service/subject/group/getSubjectGroupsTree?nDeepLevel=0&sID_Group_Activiti=OGOK_&sSubjectType=Human)
```

локальный вызов Human:
```
http://localhost:8081/wf/service/subject/group/getSubjectGroupsTree?nDeepLevel=0&sID_Group_Activiti=OGOK_&sSubjectType=Human)
```
```
https://alpha.test.region.igov.org.ua/wf/service/subject/group/getSubjectGroupsTree?sID_Group_Activiti=MHP_Depar_Finance,MJU_Dnipro_Top1&nDeepLevel=2&bIncludeRoot=false&nDeepLevelWidth=0&sSubjectType=Human
```

### Сервис по работе под референтом (спринговые и рест)
Добавление и удаление человека в персонализированную группу. Возврат всех персонализированных групп в которые я вхожу (группа персонализированная, если ее логин = существующему логину человека)

```javascript
usersService.getUserGroupMember(sLogin);
```
[Пример по ссылке](https://alpha.test.region.igov.org.ua/wf/service/action/identity/getUserGroupMember?sLogin=MJU_Dnipro_Top1_Dep1_Exec1)

```javascript
usersService.removeUserGroup(sID_Group, sLogin);
```
[Пример по ссылке](https://alpha.test.region.igov.org.ua/wf/service/action/identity/setUserGroup?sID_Group=MJU_Dnipro_Top1_Dep1_Exec9&sLogin=MJU_Dnipro_Top1_Dep1_Exec1)

```javascript
usersService.setUserGroup(sID_Group, sLogin);
```
[Пример по ссылке](https://alpha.test.region.igov.org.ua/wf/service/action/identity/removeUserGroup?sID_Group=MJU_Dnipro_Top1_Dep1_Exec9&sLogin=MJU_Dnipro_Top1_Dep1_Exec1)
...
### Проверка корректности заведения пользователей
**subjectService.getSubjectByLoginAccount(sLoginAuthor)**

Сервис проверки корректности заведения людей и органов. Обрабатывает данные пачками, размер которых задается параметром.

пример вызова:
```javascript
https://alpha.test.region.igov.org.ua/wf/service/subject/group/checkAllSubjects?nID=0&count=500&sType=Organ
```

nID=0 - начальное значение nID, с которого будет осуществляться проверка. Это номер либо из SubjectHuman либо из SubjectOrgan в зависимости от последнего параметра (Human или Organ)
count=500 - максимальный размер блока - количество строк, которые будут просматриваться, начиная я указанного nID 
sType=Organ - тип справочника Organ - SubjectOrgan или Human - SubjectHuman

В примере будут обработаны 500 людей, с идентификаторами, начинающимеся от 0. Скорость работы такого запроса - около 25 секунд.

Ответ - мапа с 4-мя ключами, где 

"HierarhyHuman_InError" - массив хьюманов с группами, в верхушке иерархии которых не стоит компания.

HierarhyOrgan_InError - тоже самое, но для органов

Human_without_company - хьюман не входит ни в один департамент (подчинение на один уровень выше)

aGroupWithoutSubject - нарушена связь сабжекта и группы


###Добавить описание
**taskForm.getLoginsFromField**
пример вызова:````javascriptvvar aGroupAddressee = taskForm.getLoginsFromField(snID_Process_Activiti, 'sTable4', null)````

####Добавить описание
***subjectService.getSubjectContacts**
```javascript
${subjectService.getSubjectContacts(execution.getProcessInstanceId(), 'sTableFamiliarize', 'sID_Group_Activiti_isFamiliarize', 'Organ', 'Email')}
```
...
Для рассылки почты используется следующая сущность:

<br><br>
   ![16_12](https://github.com/e-government-ua/i/tree/test-alpha/doc/bp/img/16_12.jpg)
...
>Изменился функционал работы блока CallActivity:
>
>1. Процесс из которого стартовали другой через данный блок теперь не попадает в историю, пока не будет отработан запущенный. После отработки они оба падают в историю.
>2. Если в документе, который стартует через данный блок на первой ЮТ права прописаны хардом на определенный логин - процесс стартоваться будет, но отображаться не будет нигде. Необходимо передавать через блок переменную sLoginAuthor и давать права на ЮТ ему.

### Отображение в истории записи о вызове документа
Чтоб в истории документа отображалась запись о том, что из него был вызван другой документ (кол активити), надо добавить следующий сервис таск перед кол активити таск:
```xml
<serviceTask id="servicetask2" name="Service Task" activiti:delegateExpression="#{SubDocumentHistoryListener}">
      <extensionElements>
        <activiti:field name="sID_Created_BP">
          <activiti:string><![CDATA[_doc_justice_22]]></activiti:string>
        </activiti:field>
      </extensionElements>
    </serviceTask>
```
Здесь _doc_justice_22 - имя шаблона, который вызывается в кол активити.<br>
Пожалуйста, БА, используйте при именовании новых чейнов предприятий правило, что имя чейна содержит не более 5 символов до знака _ <br>
![16_13](https://github.com/e-government-ua/i/tree/test-alpha/doc/bp/img/16_13.jpg)
...
[Клонирование прав участникам задания на степ](#clonerightstaskmembers)

###### steps_cloning
### Степы, клонирование прав
**Степ** - это этап жизни документа, на котором определенные пользователи имеют определенные права на него. В один момент времени документ может находиться только на одном степе за исключением параллельного ветвления.<br>
Каждый степ имеет тип, все типы степов описаны в конфигурационном файле /i/wf-base/src/main/resources/data/DocumentStepType.csv <br><br>
nID;sName;sNote;sSing<br>
0;Agree;Ознайомлення;Ознайомлено<br>
1;Accept;Узгодження;Узгоджено<br>
2;Seen;Затвердження;Затверджено<br>
3;Create;Створення;Створено<br>
4;Execute;Виконання;Виконано<br>
5;Direct;Адресацiя;Дозволено<br>
6;Edit;Редагування;Відредаговано<br>
<br><br>
Для каждого документа в его джейсоне прописываются все шаги с типами. Например,<br>
  "step_1_Creation_doc:Create": {},<br>
  "step_2_Accept:Accept": {},<br>
  "step_3_Seen:Seen": {},<br>
  "step_4_Address:Direct": {},<br>
  "step_5_Author_Edit:Edit": {},<br>
  "step_6_Acceptor_Edit:Edit": {},<br>
  "step_7_Author_Edit_Acception:Edit": {},<br>
  "step_8_Execution:Execute": {}  <br>
  <br><br>
step_1_Creation_doc - произвольное имя степа, однако его целесообразно делать контекстно понятным, потому что это имя будет использовано с скриптах. После двоеточия идет тип степа - один из описанных в конфиге DocumentStepType.csv <br>

При сознадии документа автоматически степ ставится Create. В дальнейшем переключение степа осуществляется сервисом documentStepService.setDocumentStep в скрипте:
<br>
```javascript
var snID_Process_Activiti = execution.getProcessInstanceId()
var oSwitchStep12 = documentStepService.setDocumentStep(snID_Process_Activiti, 'step_2_Accept')
```
Если на степ не склонировать права для опеределенных пользователей или груп, то никому не будет доступен тот ЮТ, перед которым переключился степ. Переход будет на ближайший степ, на который есть права у пользователей.
<br>
При создании документа на первый степ права разраются в самом БП на первом ЮТ. Это выглялит так:
<br>
```javascript
...
```
здесь ${sLoginAuthor} -  логин автора документа, `_default_btsol` - имя группы прав, которые имеет автор при создании. Эта группа прописывается в джейсоне. Например, <br>
```javascript
{
  "_": {
    "_default_btsol": {
      "sName": " ",
      "bWrite": true, 
      "aPermition": ["DeleteDocument", "CreateTask", "AnswerMessage"],    
      "asID_Field_Read": [
        "*"
      ],
      "asID_Field_Write": [
   ...
   ```
Здесь указано, что автор может подписывать документ, удалять, создавать на нет задания и отвечать на замечания. <br>
Перед ЮТ, который относится к следующему степу, необходим скрипт, который переключит степ и склонирует права пользователям на следующий степ. Клонирование прав делается сервисами **documentStepService.cloneDocumentStepSubject**, **cloneDocumentStepFromTable** и **syncDocumentSubmitedsByField**, причем последний именно синхронизирует подписантов, т.е. если какой-то был удален, то и права на этот шаг ему не раздадутся. Это используется в ситуации, когда на шаге согласования, например, автор взял документ на редактирование и удалил одного из подписантов.
<br>
Пример скрипта с переключением степа и клонированием прав при помощи синхрониззации и обычного клонирования прав:
```javascript
//переключение степа
var snID_Process_Activiti = execution.getProcessInstanceId();
var sKey_Step = 'step_2';
var oSwitchStep2 = documentStepService.setDocumentStep(snID_Process_Activiti, sKey_Step);

//клонирование прав
var sKey_GroupPostfix = '_default_btsol_approve';
var sTableName = 'sTable_Agreement';
var sID_FildTable = 'sLogin_isExecute';
var bReclone = true;

var oCloneRightsAgree = documentStepService.syncDocumentSubmitedsByField(snID_Process_Activiti, sKey_GroupPostfix, sTableName, sID_FildTable, sKey_Step, true, true)

var oCloneRightsAgree = documentStepService.syncDocumentSubmitedsByField(snID_Process_Activiti,  '_default_btsol_approve_1', 'sTable_Approve', 'sLogin_Approver', 'step_3', true, true)
var oCloneRightsAgree = documentStepService.syncDocumentSubmitedsByField(snID_Process_Activiti,  '_default_btsol_2', 'sTable_Resolution', 'sLogin_Addressee', 'step_4', true, true)
...
for (var i = 0; i < aLoginTask.length; i++){
	var sKey_GroupPostfix_New  = aLoginTask[i];
 	if(sKey_GroupPostfix_New != null){
  		var oCloneRightsAgree =  documentStepService.cloneDocumentStepSubject(snID_Process_Activiti, sKey_GroupPostfix, sKey_GroupPostfix_New, sKey_Step, bReclone)
 }
}
 ```
Чтобы исключить ситуацию, когда один человек попадает несколько раз на визирование документа (напр. автор документа и утверждающий по документу не должны попадать в согласование документа (дублироваться)). Нужно использовать сервис, который принимает логин персонализированный человека и тип проверки подписи bWrite, проверяет есть он на каком-либо степе с заданными правами на подпись (бывает bWrite тру, фолс, налл, submite).
<br>

```javascript
documentStepService.isLoginConteinsOnDocumentSteps(snID_Process_Activiti, sLogin, bWrite)
```
Пример вызова: <br>
https://alpha.test.region.igov.org.ua/wf/service/common/document/isLoginConteinsOnDocumentSteps?snID_Process_Activiti=41207539&sLogin=OGOK_010563FAV&bWrite=true

###### buttons
### Работа с кнопками
На дашборде в документе могут быть следующие кнопки.<br> 

Если в aPermition задана какая-то функция, то на фронте будет отрисовываться кнопка, соответствующая этой функции.<br><br>

Сабмитят документ:<br>
CancelDocumentSubmit - зняти підпіс<br>
RemoveDocumentStepSubjec - підпис не потрібен<br>
Seen - ознайомлений<br>
Refuse - відмовити<br>
DeleteDocument - видалити документ<br>
<br><br>
Не сабмитят документ:<br>
Delegate - делегувати<br>
AddAcceptor - додати підписанта<br>
AddVisor - ознайомити<br>
AddViewer - додати на перегляд<br>
CreateTask - створити завдання<br>
EditTask - редагувати завдання<br>
AskMessage - додати зауваження<br>
AnswerMessage - відповісти на зауваження<br>
<br><br>
При нажатии на кнопку **Delegate** предоставляется выбор логина (группы) и запускается сервис delegateDocumentStepSubject.
Для кнопок **AddAcceptor** и **AddVisor** так же предоставляется выбор логина (группы) при этом запускается сервис cloneDocumentStepSubject
(т.е. мы просто добавляем дополнительных пользователей на этот же степ и клонируем им права).
При этом в параметр sKey_GroupPostfix_New передается группа из oPermition_AddAcceptor или oPermition_AddVisor.
Если оне не заданы, то в параметр sKey_GroupPostfix_New передается дефолтная группа этого степа
и для AddVisor параметр bWrite автоматически переопределяется на false..
<br><br>
Права на эти кнопки нужно прописывать в джейсоне. По умолчанию никакие кнопки из вышеперечисленных не отображаются на документе.
В джейсоне нужно добавить строку aPermition: <br>
**"aPermition": ["Delegate","AddAcceptor","AddVisor"","AddViewer"]**<br>
где указать только те кнопки, которые нужно отобразить. В данном случае - это делегирование, добавление на подпись, на ознакомление и просмотр.<br>
Для кнопок, в которых осуществляется выбор пользователя, нужно прописывать также группу, из которой подгружать пользователей для выбора, и права, которыми будет наделен выбранные пользоватлей(и). Для этого в джейсоне нужно прописать следующую строку:
<br><br>
"oPermitions_ButtonName": {"sKeyGroupe_Source":"RightGroupName", "sGroup_Activiti":"FromGroupName"} <br><br>

где ButtonName - имя кнопки, RightGroupName - название группы доступа, которая прописывается в этом же джейсоне, FromGroupName - название группы, из которой будут подгружены пользователи (из SubjectGroup).<br><br>

Например,<br>

"oPermitions_AddViewer": {"sKeyGroupe_Source":"_default_btsol_view", "sGroup_Activiti":"btsol_head,OGOK_,MGOK_,ZCPK_"}<br>

Если в массиве aPermition указан один из элементов AddAcceptor/AddVisor/AddViewer/Delegate, но при этом не задан объект oPermitions_AddAcceptor/AddVisor/AddViewer/Delegate, тогда по умолчанию задается дефолтная группа для клонирования прав.<br>

Если в массиве aPermition указан элемент AddVisor, но при этом не задан объект oPermitions_AddVisor, тогда флаг bWrite автоматически переопределяется на false, независимо от того что стоит в дефолтной группе.<br>

Если в массиве aPermition указан элемент AddViewer, но при этом не задан объект oPermitions_AddViewer, тогда флаг bWrite автоматически переопределяется на null, независимо от того что стоит в дефолтной группе.<br>
...
```javascript
{
  "_": {
    "_default_btsol": {
     ...
     "_default_btsol_avtor_edit": {
      "sName": " ",
      "bWrite": true,
      "aPermition": ["DeleteDocument", "CreateTask", "EditTask", "AskMessage"],      
      "asID_Field_Read": [
        "*"
      ],
      "asID_Field_Write": [
    ...
    "_default_btsol_approve": {
      "sName": " ",
      "bWrite": true,
      "aPermition": ["AddViewer", "Delegate", "AddAcceptor", "AddVisor", "CancelDocumentSubmit", "RemoveDocumentStepSubject", "AskMessage"],
      "oPermitions_AddAcceptor": {"sKeyGroupe_Source":"_default_btsol_approve", "sGroup_Activiti":"btsol_head,OGOK_,MGOK_,ZCPK_"},
      "oPermitions_AddViewer": {"sKeyGroupe_Source":"_default_btsol_view", "sGroup_Activiti":"btsol_head,OGOK_,MGOK_,ZCPK_"}, 
       "oPermitions_AddVisor": {"sKeyGroupe_Source":"_default_btsol_read", "sGroup_Activiti":"btsol_head,OGOK_,MGOK_,ZCPK_"},
      "oPermitions_Delegate": {"sGroup_Activiti":"btsol_head,OGOK_,MGOK_,ZCPK_"},    
      "asID_Field_Read": [
        "*"
      ],
      "asID_Field_Write": [
     ...
     "_default_btsol_approve_1": {
      "sName": " ",
      "bWrite": true,
      "aPermition": ["AddViewer", "Delegate", "AddAcceptor", "AskMessage"],
      "oPermitions_AddAcceptor": {"sKeyGroupe_Source":"_default_btsol_approve_1", "sGroup_Activiti":"btsol_head,OGOK_,MGOK_,ZCPK_"},
      "oPermitions_AddViewer": {"sKeyGroupe_Source":"_default_btsol_view", "sGroup_Activiti":"btsol_head,OGOK_,MGOK_,ZCPK_"}, 
      "oPermitions_Delegate": {"sGroup_Activiti":"btsol_head,OGOK_,MGOK_,ZCPK_"},    
      "asID_Field_Read": [
        "*"
    ...
    "_default_btsol_2": {
      "sName": " ",
      "bWrite": true,
      "aPermition": [ "Seen", "AddViewer", "Delegate", "CreateTask", "CancelDocumentSubmit", "RemoveDocumentStepSubject", "AskMessage", "Refuse"],
      "oPermitions_AddViewer": {"sKeyGroupe_Source":"_default_btsol_view", "sGroup_Activiti":"btsol_head,OGOK_,MGOK_,ZCPK_"},  
       "oPermitions_Delegate": {"sGroup_Activiti":"btsol_head,OGOK_,MGOK_,ZCPK_"},
      "asID_Field_Read": [
        "*"
...
Теперь так же доступна подгрузка файлов в тейбл. Джейсон с таблицей должен выглядеть так:
```xml
{
  "aField": [
    {
...
[вернуться...](#getdirdatafromscript)
   ...
   ![16_10](https://github.com/e-government-ua/i/tree/test-alpha/doc/bp/img/16_10.jpg)
   <br><br><br>
   ![16_11](https://github.com/e-government-ua/i/tree/test-alpha/doc/bp/img/16_11.jpg)
   
   
###### clonerightstaskmembers
### Права участникам задания на степ

Скрипт, который раздает права участникам задания на степ sKey_Step. Группа прав _default_btsol_approve прописана в джейсоне БП.
```javaScript
var sKey_GroupPostfix = '_default_btsol_approve';
var aLoginTask = processSubjectTaskService.getProcessSubjectByDocument(snID_Process_Activiti,  null);
for (var i = 0; i < aLoginTask.length; i++){
