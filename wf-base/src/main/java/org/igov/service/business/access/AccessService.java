...
import static org.igov.service.exception.ExceptionMessage.ACCESS_DENIED;
import static org.igov.service.exception.ExceptionMessage.SESSION_ENDED;
...
public class AccessService implements ApplicationContextAware {
     ...
     * authentificate user in actifiti and set session Logins
     */
    public LoginResponseI login(String sLogin, String sPassword, HttpServletRequest oRequest) throws AccessServiceException{
        User oUser = oIdentityService.createUserQuery().userId(sLogin).singleResult();
        if (oUser != null) {
            if ((generalConfig.isSelfTest() && sPassword.equals(" ")) || 
                    (oIdentityService.checkPassword(sLogin, sPassword))) 
            {
                LOG.info("test empty login...");
                HttpSession oSession = oRequest.getSession(true);
                oSession.setMaxInactiveInterval(7200);
                oSession.setAttribute("sLogin", sLogin);
                oSession.setAttribute("sLoginReferent", sLogin);
                oSession.setAttribute("Browser", getUserAgent(oRequest.getHeader("User-Agent")));
                LOG.info("!!!---!!!oSession: " + oSession.getId() + " sLogin: " + oSession.getAttribute("sLogin"));
    ...
    /**
     * @author Yegor Kovylin
     * @param sLoginPrincipal - login of referent user
     * @param oRequest
     * @return
     * set principal login in session
     * @throws Exception 
     */
    public Map<String,Object> setsLoginPrincipal(String sLoginPrincipal, HttpServletRequest oRequest) throws Exception{
        ...
        if(oSession != null){
            ...
            //Set<String> aUserGroup = oUsersService.getUserGroupMember(sLogin);
            //if (aUserGroup.contains(sLoginPrincipal)) {
                
                LOG.info("oRequest sLoginPrincipal sLogin{}", oSession.getAttribute("sLogin"));
                oSession.setAttribute("sLogin", sLoginPrincipal);
                LOG.info("setsLoginPrincipal is {} ", oSession.getAttribute("sLogin"));
                
                Map<String,Object> mResultUser = oUsersService.getmUserProperty(sLoginPrincipal);
                return mResultUser;
            //} else {
            //    throw new RuntimeException(ACCESS_DENIED);
            //}
        }else{
            throw new RuntimeException(SESSION_ENDED);
        }
    }
    ...
    private String getUserAgent(String userAgent) {
        ...
        if (userAgent != null) {
            if (userAgent.contains("Mozilla/5.0")
                    || userAgent.contains("Safari/530.17")
                    || userAgent.contains("Safari/419.3")
                    || userAgent.contains("Opera 8")) {
                result = "Opera";
            } else if (userAgent.contains("Windows 98")) {
                result = "MSIE 5.0";
            } else if (userAgent.contains("MSIE") && userAgent.contains(";")) {
                try {
                    result = userAgent.substring(userAgent.indexOf("MSIE"), userAgent.indexOf(";", userAgent.indexOf("MSIE")));
    ...
    public String getSessionLogin(String sLogin, String sLoginPrincipal, HttpServletRequest oRequest) {
        String sResultLogin = null;
        HttpSession oSession = oRequest.getSession(false);
        if (oSession != null) {
            LOG.info("oSession isn't null");
            if (oSession.getAttribute("sLogin") != null) {
                if (oSession.getAttribute("sLoginReferent") != null) {
                    sResultLogin = (String) oSession.getAttribute("sLoginReferent");
                    LOG.info("sLoginReferent {}", sResultLogin);
                } else {
                    sResultLogin = (String) oSession.getAttribute("sLogin");
                    LOG.info("sLogin {}", sResultLogin);
                }
            } else {
                throw new RuntimeException(ACCESS_DENIED);
            }
        } else {
            LOG.info("oSession is null");
            sResultLogin = sLoginPrincipal == null ? sLogin : sLoginPrincipal;
        }

        LOG.info("sResultLogin {}", sResultLogin);
        return sResultLogin;
    }
}
