...
public class HistoryEventDaoImpl extends GenericEntityDao<Long, HistoryEvent> implements HistoryEventDao {
    ...
    @Override
	public HistoryEventVO getHistoryEventsByProcessId(String snID_process, Integer nSize, Integer nStart) {
		long nID_Process = Long.valueOf(snID_process);
		List<HistoryEvent> aoHistoryEvent = new ArrayList<>();
		int nTotalCount = 0;
		Query query = getSession().createQuery(
				"SELECT event FROM HistoryEvent event, HistoryEvent_Service eventService"
						+ " WHERE event.oHistoryEvent_Service.id = eventService.id"
						+ " AND eventService.nID_Process = :nID_Process ORDER BY event.date DESC")
				.setParameter("nID_Process", nID_Process)
				.setReadOnly(true);
		ScrollableResults resultScroll = query.scroll();
		resultScroll.first();
		resultScroll.scroll(nStart);
		int i = 0;
		while (nSize > i++ && resultScroll.next()) {
			aoHistoryEvent.add((HistoryEvent) resultScroll.get(0));
		}
		resultScroll.last();
		nTotalCount = resultScroll.getRowNumber() + 1;

		HistoryEventVO oHistoryEventVO = new HistoryEventVO();
		oHistoryEventVO.setAoHistoryEvent(aoHistoryEvent);
		oHistoryEventVO.setnTotalCount(nTotalCount);
		oHistoryEventVO.setnStart(nStart);
		oHistoryEventVO.setnSize(nSize);

		return oHistoryEventVO;
	}
