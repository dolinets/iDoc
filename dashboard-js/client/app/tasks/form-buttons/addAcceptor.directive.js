...
angular.module('dashboardJsApp').directive('acceptorSelect', ['$http', 'tasks', 'Auth', '$rootScope', '$timeout',
  'DocumentsService', 'Modal', 'CurrentServer', '$cookies',
  function($http, tasks, Auth, $rootScope, $timeout, DocumentsService, Modal, CurrentServer, $cookies) {
    return {
      restrict: 'EA',
      templateUrl: 'app/tasks/form-buttons/addAcceptor.html',
        link: function (scope, element, attrs, ngModel) {
          ...
          scope.refreshUserData = function (sFind) {
            for( var j=0; j<scope.documentLogins.length; j++ ) {
              ...
              for( var l=0; l<users.length; l++ ){
                if(users[l].sLogin === login.id) {
                  queryParams.params.sID_Group_Activiti = $rootScope.docRights.AddAcceptor_Group_Activiti;
                  queryParams.params.sSubjectType = 'Human';
                  queryParams.params.nDeepLevel = 0;
                  scope.sKey_Group = users[l].sLogin;
                  break;
                }
              }
            }
            ...
            $http.get('./api/subject-role', queryParams).then(function (res) {
              ...
              scope.acceptorUsersList = response;
            });
          }

          scope.onSelectUser = function (user, $scope) {
          ...
          scope.safeAcceptor = function () {
            scope.modalSpinner = true;
           if (scope.accepts.selected) {
            ...
            DocumentsService.addAcceptorToDoc(scope.params).then(function (res) {
               $timeout(function(){
                scope.$apply();
                Modal.inform.success()('Підписанта успішно додано');
                scope.updateListOfAcceptors(res);
              });
              scope.accepts.selected = null;
