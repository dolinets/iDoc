...
function DropdownGroupController($http) {
  var vm = this
    , groups
    , allGroups
    , groupsToAdd = null
    ...
    , groupToRemove = null
    , groupsList = []
    , dropdownUserList = $('.dropdown-users-list');
  ...
  vm.groupsSearch = function (sFind, isPasteEvent) {
    $http({
      method: 'GET',
      ...
      if (isPasteEvent && !dropdownUserList.hasClass('open')) {
        setTimeout(function () {
          angular.element('.user-dropdown-caret').triggerHandler('click');
        })
      }
    });
  };
