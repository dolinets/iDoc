package org.igov.model.object;

import com.fasterxml.jackson.annotation.JsonProperty;
import io.swagger.annotations.ApiModel;
import io.swagger.annotations.ApiModelProperty;
import org.igov.model.core.AbstractEntity;

import javax.persistence.Column;

/**
 * @author grigoriy-romanenko
 */
@javax.persistence.Entity

@ApiModel(description="Классификация видов целевого назначения земель")
public class ObjectEarthTarget extends AbstractEntity {

    @JsonProperty(value = "sID_UA")
    @Column(length = 10, unique = true, nullable = false)
    @ApiModelProperty(value = "Уникальная строка-ИД позиции классификатора видов целевого назначения земель", required = true)
    private String sID_UA;

    @JsonProperty(value = "sName_UA")
    @Column(length = 500, unique = true, nullable = false)
    @ApiModelProperty(value = "Название позиции классификатора видов целевого назначения земель", required = true)
    private String sName_UA;

    public String getsID_UA() {
        return sID_UA;
    }

    public void setsID_UA(String sID_UA) {
        this.sID_UA = sID_UA;
    }

    public String getsName_UA() {
        return sName_UA;
    }

    public void setsName_UA(String sName_UA) {
        this.sName_UA = sName_UA;
    }
}
