...
public class ActionEscalationCommonController {

    private static final Logger LOG = LoggerFactory.getLogger(ActionEscalationCommonController.class);
    ...
    @ResponseBody
    public String runEscalationRule(
            @ApiParam(value = "номер-ид правила эскалации", required = true) @RequestParam(value = "nID") Long nID)
            throws Exception {
        return oEscalationService.runEscalationRule(nID, generalConfig.getSelfHost());
    }
    ...
    public void runEscalationAll() throws CommonServiceException {
        oEscalationService.runEscalationAll(null);
    }
    ...
    @ApiOperation(value = "Возвращает данные по эскалации")
    @RequestMapping(value = "/getEscalationTaskData", method = RequestMethod.GET)
    @ResponseBody
    public  Map<String, Object> getEscalationTaskData(
            @ApiParam(value = "номер-ИД процесса", required = false) @RequestParam(value = "nID_Process", required = false) Long nIdProcess) throws CommonServiceException, Exception 
    {
       Task oTask = oTaskService.createTaskQuery().processInstanceId(nIdProcess.toString()).singleResult();
       return oEscalationService.getTaskData(oTask);
    }
