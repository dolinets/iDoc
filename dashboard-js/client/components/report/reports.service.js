angular.module('dashboardJsApp').factory('reports', function tasks($http, $httpParamSerializer, Auth) {
  ...
  return {
        ...
        if (result.data != '' && !(result.data.code && result.data.code === 'BUSINESS_ERR')) {
          _.each(result.data.split("\n"), function (line) {
            ...
            if (!!sr && sr.length >= 2) {
              ...
              if (dataArray.hasOwnProperty(propKey)) {
    ...
    getExportLink: function(obj) {
      var baseUrl = '/api/reports/downloadTaskStatusData?';

      var params = {
        sID_BP: obj.sBP.oSubjectRightBP.sID_BP,
        sFileName: transliterate(obj.sBP.sName_BP),
        dateAt: obj.from,
        dateTo: obj.to
      };
