...
angular.module('dashboardJsApp')
	...
	.run(
		function($rootScope, $location, $cookieStore, Auth, Idle, Modal, $state, $http) {
			var isKeepAliveAsked = false;

      $http.get('/api/env/get-env-config').success(function (response) {
        $rootScope.ProjectRegion_MainPage_bTasksOnly = response.oOrganisation.ProjectRegion_MainPage_bTasksOnly;
        $rootScope.ProjectRegion_MainPage_biDocOnly = response.oOrganisation.ProjectRegion_MainPage_biDocOnly;
        $rootScope.ProjectRegion_LogonPage_sName = response.oOrganisation.ProjectRegion_LogonPage_sName;
        $rootScope.ProjectRegion_LogonPage_sNote = response.oOrganisation.ProjectRegion_LogonPage_sNote;
        $rootScope.ProjectRegion_LogonPage_sContacts = response.oOrganisation.ProjectRegion_LogonPage_sContacts;
        $rootScope.ProjectRegion_StartView_bDocuments = response.oOrganisation.ProjectRegion_StartView_bDocuments;
        $rootScope.bHelpDesk = response.oOrganisation.bHelpDesk;
      });
      ...
      $rootScope.$on('$stateChangeStart', function(event, toState, toParams, fromState, fromParams, options) {
        if (toState && toState.access && toState.access.requiresLogin) {
          if (!Auth.isLoggedIn()) {
            ...
            if (location.indexOf('sID_Order') > -1) {
            	$rootScope.continueUrl = location;
            }
