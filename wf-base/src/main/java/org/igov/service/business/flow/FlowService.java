...
public class FlowService implements ApplicationContextAware {
    ...
    @Autowired
    public GeneralConfig oGeneralConfig;
    ...
    public void addFlowSlowTicketToResult(List<TaskDataVO> aTaskDataVO,
            SimpleDateFormat oDateFormat, FlowSlotTicket oFlowSlotTicket, Task oTask) {
        
        TaskDataVO oTaskDataVO = new TaskDataVO();
        Map<String, Object> mFlowSlotTicketData = new HashMap<>();
        ...
        mFlowSlotTicketData.put("nID", oFlowSlotTicket.getId().toString());
        mFlowSlotTicketData.put("nID_FlowSlot", !oFlowSlotTicket.getaFlowSlot().isEmpty()
                ? oFlowSlotTicket.getaFlowSlot().get(0).getId().toString() : "");
        mFlowSlotTicketData.put("nID_Subject", oFlowSlotTicket.getnID_Subject().toString());
        Date oDateStart = new Date(oFlowSlotTicket.getsDateStart().getMillis());
        mFlowSlotTicketData.put("sDateStart", oDateFormat.format(oDateStart));
        Date oDateFinish = new Date(oFlowSlotTicket.getsDateFinish().getMillis());
        mFlowSlotTicketData.put("sDateFinish", oDateFormat.format(oDateFinish));
        Date oDateEdit = new Date(oFlowSlotTicket.getsDateEdit().getMillis());
        mFlowSlotTicketData.put("sDateEdit", oDateFormat.format(oDateEdit));

        oTaskDataVO.setProcessDefinitionId(oTask.getProcessDefinitionId());
        oTaskDataVO.setCreateTime(oTask.getCreateTime());
        oTaskDataVO.setsCreateTime(oDateFormat.format(oTask.getCreateTime()));
        oTaskDataVO.setName(oTask.getName());
        oTaskDataVO.setId(oTask.getId());
        oTaskDataVO.setProcessInstanceId(oTask.getProcessInstanceId());
        oTaskDataVO.setnOrder(ToolLuna.getProtectedNumber(Long.parseLong(oTask.getProcessInstanceId())));
        oTaskDataVO.setsID_Order(oGeneralConfig.getOrderId_ByProcess(Long.parseLong(oTask.getProcessInstanceId())));
        oTaskDataVO.setFlowSlotTicketData(mFlowSlotTicketData);

        aTaskDataVO.add(oTaskDataVO);
    }

    public void addFlowSlowTicketToResult_DMS(List<TaskDataVO> aTaskDataVO,
            SimpleDateFormat oDateFormat, String sTicket_Number, String sTicket_Code, String sDate, Task oTask) {
        ...
        mFlowSlotTicketData.put("nID", sTicket_Number);
        mFlowSlotTicketData.put("nID_FlowSlot", sTicket_Code);
        mFlowSlotTicketData.put("nID_Subject", "0");
        mFlowSlotTicketData.put("sDateStart", sDate);
        mFlowSlotTicketData.put("sDateFinish", sDate);
        mFlowSlotTicketData.put("sDateEdit", sDate);
    ...
    /**
     * Проверить nID_Flow, и если (nID_Flow == null) - попытаемся его определить
     *
     ...
     * @throws RecordNotFoundException если <br> nID_Flow==null and sID_BP==null
     * <br> или определить номер-ИД потока по заданному sID_BP - не удалось
     */
     ...
     * @param nID_Flow - номер-ИД потока (обязательный если нет sID_BP)
     ...
     */
    public TaskDataResultVO getFlowSlotTickets(String sLogin, Boolean bEmployeeUnassigned, String sDateFilter)
            throws ParseException {
        LOG.info("Start getFlowSlotTickets sLogin={}, sDateFilter={}", sLogin, sDateFilter);
        ...
        try {
            ...
            if (sDateFilter != null) {
                ...
                LOG.debug("Converted sDateFilter to oDateFilter! (sDateFilter={},oDateFilter:{})", sDateFilter, oDateFilter);
            }
            ...
            for (Task oTask : aTask) {
                if (oTask.getProcessInstanceId() != null) {
                    ...
                    for (FormProperty oProperty : aProperty) {
                        LOG.debug("aProperty id={}, name={}, type={}, value={}",
                                oProperty.getId(), oProperty.getName(), oProperty.getType(), oProperty.getValue());
                        if (oProperty.getType() instanceof QueueDataFormType) {
                            ...
                            if (sValue != null && !"".equals(sValue.trim()) && !"null".equals(sValue.trim())) {
                                ...
                                if (bFlowOut(sID_Type)) {//Нет ни какой обработки т.к. это внешняя ЭО
                                    ...
                                    Date oDate = null;
                                    if (sDate != null) {
                                        LOG.debug("Converting sDate to oDate! (sDate={}) ", sDate);
                                        oDate = new SimpleDateFormat("yyyy-MM-dd").parse(sDate);
                                    }

                                    if (oDate == null || oDateFilter == null || (DateUtils.isSameDay(oDate, oDateFilter))) {
                                        addFlowSlowTicketToResult_DMS(aTaskDataVO, oDateFormat, sTicket_Number, sTicket_Code, sDate, oTask);
                                    }
                                }
                            }
                        }
                    ...
                    LOG.warn("Task with ID:{} has null process instance id value", oTask.getId());
                }
            }
            LOG.debug("Will check tasks which belong to process definition IDs:{}", mTask.keySet());
            ...
            if (mTask.isEmpty()) {
                ...
                if (aFlowSlowTicket != null) {
                    Collections.sort(aFlowSlowTicket, (oTicket1, oTicket2)
                            -> oTicket1.getsDateStart().compareTo(oTicket2.getsDateStart()));
                    ...
                    for (FlowSlotTicket oFlowSlotTicket : aFlowSlowTicket) {
                        if (mTask.keySet().contains(oFlowSlotTicket.getnID_Task_Activiti())) {
                            ...
                            if (oDateFilter == null || (DateUtils.isSameDay(oFlowSlotTicket.getsDateStart().toDate(), oDateFilter))) {
                                addFlowSlowTicketToResult(aTaskDataVO, oDateFormat, oFlowSlotTicket, oTask);
                            } else {
                                LOG.debug("Skipping flowSlot {} for task:{} as they have not valid  start-end date {}:{}",
            ...
            oTaskDataResultVO.setSort("createTime");
