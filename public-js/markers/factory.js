angular.module('iGovMarkers')
  .factory('iGovMarkers', function ($http, $q, iGovMarkersDefaults, iGovMarkersSchema) {
    ...
    return {
      ...
      interpolateString: function (strPattern, objData, startSymbol, endSymbol, arrKeys, arrFormProperties) {
        ...
        function addSubstring(index) {
          ...
          // for(var key in arrFormProperties) {
          //     if(arrFormProperties.hasOwnProperty(key) && arrFormProperties[key].type === 'table') {
          //         var rows = arrFormProperties[key].aRow;
          //         for(var rows in row) {
          //           var nRowNumber = rows[row];
          //         }
          //     }
          // }
        ...
        for (var arrInd = 0; arrInd < aSubstrings.length; arrInd++) {
          angular.forEach(arrKeys, function (objDataKey) {
            ...
            if (objDataKey === aSubstrings[arrInd]) {
              if(objDataKey.indexOf('nTable') > -1) {
                bIsTable = true;
              }
               if (objData[objDataKey] && objData[objDataKey].value && angular.isString(objData[objDataKey].value)) {
                 bIsSetEnumValue = false;
                if (arrFormProperties) {
                  angular.forEach(arrFormProperties, function (oProperty) {
                    if (oProperty.type && oProperty.type === 'enum' && oProperty.id === objDataKey && !bIsSetEnumValue) {
              ...
              } else if (objData[objDataKey] && angular.isString(objData[objDataKey])) {
                  result.value = result.value + objData[objDataKey];
                  result.differentTriggered = true;
              } else if(bIsTable) {
                   var oTable = traverseTable(arrFormProperties, objDataKey);
                   result.value = result.value + oTable.value;
               }
      ...
      checkOrderValue: function (sId) {
        var deferred = $q.defer();
        $http.get('/api/markers/checkOrderValue/' + sId).then(function(result){
          try {
            var aResult = JSON.parse(result.data);
          } catch (err) {
            deferred.reject(false);
          }

          if(angular.isArray(aResult) && aResult.length > 0){
