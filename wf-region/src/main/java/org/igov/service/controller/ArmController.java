...
public class ArmController {
    ...
    @ApiOperation(value = "Получение 441 номера по номеру заявки и логинк исполнителя из таблицы arm.dbo.TK", notes = "##### Пример:\n"
            + "https://alpha.test.region.igov.org.ua/wf/service/arm/getMaxValue442\n")
    @RequestMapping(value = "/getNumber441", method = RequestMethod.GET)
    @ResponseBody
    public Integer getNumber441(@ApiParam(value = "Номер заявки из айдок", required = true) @RequestParam(value = "sID_Order") String sID_Order,
            @ApiParam(value = "Логин исполнителя", required = true) @RequestParam(value = "sExecutor") String sExecutor)
            throws Exception {
        Integer nNumber441 = null;
        try {
            nNumber441 = armService.getNumber441(sID_Order, sExecutor);
