...
/**
 * User: goodg_000 Date: 06.10.2015 Time: 23:08
 */
@WebAppConfiguration
...
public class AccessCommonControllerScenario {
    ...
    @Ignore
    @Test
    public void shouldSuccessfullyVerifyContactEmail() throws Exception {

        Map<String, String> res;

        String getJsonData = mockMvc.perform(get("/access/verifyContactEmail").
                param("sQuestion", "test@igov.org.ua")).
                andExpect(status().isOk()).
                andExpect(content().contentType(APPLICATION_JSON_CHARSET_UTF_8)).
                andReturn().getResponse().getContentAsString();
        res = JsonRestUtils.readObject(getJsonData, HashMap.class);
        Assert.assertEquals(res.get("bVerified"), "true");
        
        getJsonData = mockMvc.perform(get("/access/verifyContactEmail").
                param("sQuestion", "testigov.org.ua")).
                andExpect(status().isOk()).
                andExpect(content().contentType(APPLICATION_JSON_CHARSET_UTF_8)).
                andReturn().getResponse().getContentAsString();
        res = JsonRestUtils.readObject(getJsonData, HashMap.class);
        Assert.assertEquals(res.get("bVerified"), "false");
    }
