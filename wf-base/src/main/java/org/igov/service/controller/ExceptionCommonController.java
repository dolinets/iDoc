...
import org.igov.model.action.task.core.entity.TaskErrorResponse;
...
public class ExceptionCommonController {
    ...
    @ExceptionHandler(value = CommonTaskException.class)
    @ResponseBody
    public ResponseEntity<String> catchCommonTaskException(CommonTaskException oException) {
        new Log(oException, LOG)
                ._Case("REST_API")
                ._Head("REST API Exception")
                ._Status(LogStatus.ERROR)
                ...
                ;
        return JsonRestUtils.toJsonResponse(oException.getHttpStatus(), 
                new TaskErrorResponse(oException.getErrorCode(), oException.getMessage(),
                        oException.getsID_Order(), oException.getsDocumentStatus()));
    }
