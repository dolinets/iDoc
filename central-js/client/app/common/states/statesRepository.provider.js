...
angular.module('appBoilerPlate').provider('statesRepository', function StatesRepositoryProvider() {
  ...
  var modeModel = {
    ...
    "idoc_etg": {
      "header": "etg.header.html",
      "footer": "etg.footer.html",
      "placesID": []
      ,"asOrgan": ['Energy trade group']
      ,"bSearch": false
      ,"anID_CatalogCategoryShowAll": [1]
      ,"nID_Place_Profile": 0
    }
  ...
  this.init = function (domen) {
    ...
    if (domen.split(':')[0] !== 'localhost') {
      if (domen.indexOf('kievcity') >= 0 || domen.indexOf('kiev.test.') >= 0 || domen.indexOf('test.kiev.') >= 0) {
        ...
        //dfs.igov.org.ua
        this.mode = 'dfs';
      } else if (domen.indexOf('idoc.etg') >= 0 || domen.indexOf('idoc-etg') >= 0 || domen.indexOf('idoc-etg.test') >= 0) {
        //idoc-etg.test.igov.org.ua
        this.mode = 'idoc_etg';
      } else {
  ...
  this.index = function () {
    return {
      ...
      views: {
        ...
        'main@': {
          templateUrl: this.isDFS() ? 'app/service/index/services.html' : this.isETG() ? 'app/service/index/no.services.html' : 'app/service/index/new.services.html',
