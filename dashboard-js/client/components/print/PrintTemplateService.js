...
angular.module('dashboardJsApp').service('PrintTemplateService', ['tasks', 'FieldMotionService', 'PrintTemplateProcessor', '$q', '$templateRequest', '$stateParams', function(tasks, FieldMotionService, PrintTemplateProcessor, $q, $templateRequest, $stateParams) {
  ...
  function findPrintTemplate (form, sCustomFieldID) {
    ...
    var printTemplateResult = form.filter(function (item) {
      ...
      if ($stateParams.type === 'docHistory'){
        bPrintform = item.sId === s;
      } else {
        bPrintform = item.id === s;
      }
      return bPrintform;
      //return item.id === s;
    });
    var retval;
    if ($stateParams.type === 'docHistory'){
      retval = printTemplateResult.length !== 0 ? printTemplateResult[0].sName.replace(/\[pattern(.+)\].*/, '$1') : "";
    } else {
      retval = printTemplateResult.length !== 0 ? printTemplateResult[0].name.replace(/\[pattern(.+)\].*/, '$1') : "";
    }
    return retval;
  };
  ...
  var service = {
      ...
      if (markerExists){
        if ($stateParams.type === 'docHistory') {
          templates = form.filter(function (item) {
            var result = false;
            if ((item.sId && item.sId.indexOf('sBody') >= 0)
              && (!FieldMotionService.FieldMentioned.inShow(item.sId)
                || (FieldMotionService.FieldMentioned.inShow(item.sId)
                  && FieldMotionService.isFieldVisible(item.sId, form)))) {
              result = true;
              if (item.oValue && item.oValue.trim().length > 0 && item.oValue.length <= 100){
                item.displayTemplate = item.oValue;
              } else {
    ...
    getPrintTemplateByObject: function(form, printTemplateObject ) {
      ...
      if(!angular.isDefined(loadedTemplates[printTemplateObject.sPatternPath])) {
         tasks.getPatternFile(printTemplateObject.sPatternPath).then(function(originalTemplate) {
           ...
           parsedForm = PrintTemplateProcessor.getPrintTemplate(form, originalTemplate);
        ...
        parsedForm = PrintTemplateProcessor.getPrintTemplate(form, loadedTemplates[printTemplateObject.sPatternPath]);
        parsedForm = PrintTemplateProcessor.populateTableField( parsedForm, printTemplateObject );
        deferred.resolve(parsedForm);
      }
      return deferred.promise;
    ...
    getPrintTemplate: function(form, printTemplateName, signers, issues) {
      var deferred = $q.defer();
      ...
      if (!angular.isDefined(loadedTemplates[printTemplatePath])) {
        ...
        tasks.getPatternFile(printTemplatePath).then(function(originalTemplate){
          ...
          parsedForm = PrintTemplateProcessor.getPrintTemplate(form, originalTemplate, signers, issues);
          deferred.resolve(parsedForm);
        }, function() {
        ...
        parsedForm = PrintTemplateProcessor.getPrintTemplate(form, loadedTemplates[printTemplatePath]);
        deferred.resolve(parsedForm);
      }
      // return promise
      return deferred.promise;
