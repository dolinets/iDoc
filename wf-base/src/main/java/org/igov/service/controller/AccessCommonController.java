...
/**
 * User: goodg_000 Date: 06.10.2015 Time: 22:57
 */
@Controller
@Api(tags = {"AccessCommonController — Доступ общий (права доступов к сервисам)"})
@RequestMapping(value = "/access")
public class AccessCommonController implements ExceptionMessage{
     ...
     * true - Пользователь авторизирован false - Имя пользователя или пароль
     * некорректны
     *
     * @param login - Логин пользователя
     * @param sPassword - Пароль пользователя
     * @return {"session":"true"} -- Пользователь авторизирован OR
     * {"session":"false"}- Имя пользователя или пароль некорректны
    ...
    @ApiResponses(value = {
        @ApiResponse(code = 200, message = "Возращает признак успеха/неудачи входа:\n "
    ...
    LoginResponseI login(
            ...
            @ApiParam(value = "Строка пароль пользователя", required = true) @RequestParam(value = "sPassword") String sPassword,
            HttpServletRequest oRequest)
            throws AccessServiceException {
        return oAccessService.login(sLogin, sPassword, oRequest);
    }

    @RequestMapping(value = {"/setsLoginPrincipal"}, method = RequestMethod.GET)
    public @ResponseBody
    Map<String,Object> setsLoginPrincipal(
            @ApiParam(value = "Строка пароль пользователя - референта", required = true) @RequestParam(value = "sLoginPrincipal") String sLoginPrincipal,
            HttpServletRequest oRequest) throws Exception {
        
        return oAccessService.setsLoginPrincipal(sLoginPrincipal, oRequest);
    }
    ...
    @ApiOperation(value = " Аутентификация пользователя. Логаут пользователя (наличие cookie JSESSIONID)", notes
            ...
            + "\n```\n")
    @ApiResponses(value = {
        @ApiResponse(code = 200, message = "Возращает JSESSIONID")})
    ...
    Map<String, String> verifyContactEmail(
            ...
            @ApiParam(value = "Строка ответ (код )", required = false) @RequestParam(value = "sAnswer", required = false) String sAnswer)
     ...
     * @param sData — Строка параметров к сервису (опциональный параметр, формат
     * передачи пока не определен). Если задан бин sHandlerBean (см. ниже) то он
     * может взять на себя проверку доступности сервиса для данного набора
     * параметров.
     */
    ...
    @ApiResponses(value = {
        @ApiResponse(code = 500, message = "Ошибка бизнес-процесса")
        ,
    ...
    public ResponseEntity hasAccessServiceLoginRight(
            ...
            @ApiParam(value = "Метод доступа к свервису (GET или POST или другиие)", required = true) @RequestParam(value = "sMethod") String sMethod)
    ...
    @ApiResponses(value = {
        @ApiResponse(code = 200, message = "Результирующий обьект роли логина")})
    ...
    @ApiResponses(value = {
        @ApiResponse(code = 200, message = "Результирующий обьект роли")})
    ...
    @ApiResponses(value = {
        @ApiResponse(code = 200, message = "Результирующий объект-права на сервис")})
