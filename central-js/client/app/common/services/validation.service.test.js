describe('ValidationService Tests', function() {
  ...
  it('EndPeriod validation:', function() {
    var scope = $rootScope.$new();
    var markers = {
      validate: {
        EndPeriod: {
          aField_ID: ['end_date'],
          aDateBegin_ID: ['begin_date']
        }
      }
    };
    $document.find('body').append(
      $compile('<form name="taskForm" action="#">'
              +'<input name="begin_date" ng-model="sDateBegin" />'
              +'<input name="end_date" ng-model="sDateEnd" />'
              +'</form>') (scope));
    scope.sDateBegin = '01/01/1980';
    scope.sDateEnd = '01/01/1985';
    scope.$digest();
    validationService.validateByMarkers(scope.taskForm, markers, true, {}, false);
    expect(scope.taskForm.$valid).toBe(true);
    expect(scope.taskForm.$invalid).toBe(false);
  });
