      ...
      var httpInterceptors = {
        ...
        'responseError': function(rejection) {
          ...
          if(rejection.status == 401) {
            ...
            return $q.reject(rejection);
          } else if(rejection.config && rejection.config.params && rejection.config.params.bNoShowErrorWindow){
            return rejection;
          }
          try {
          ...
          if(rejection.data.errorCode !== "documentChanged") {
            Modal.inform.error()(rejection.data.message || rejection.data.serverMessage || (rejection.statusText === '' ? 'Виникла помилка: ' + rejection.status : rejection.statusText));

            if (rejection.data.message && rejection.data.message.indexOf('Немає доступу на данну дiю') > -1) {
